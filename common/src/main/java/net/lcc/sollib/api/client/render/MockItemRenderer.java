...
public class MockItemRenderer {
    protected static final SolLogger LOG = new SolLogger("Sol/Render");
    ...
    public static Boolean[][] loadPixelData(ResourceLocation texture, int alphaThreshold) {
        ...
        Optional<Resource> optional = Minecraft.getInstance().getResourceManager().getResource(texture);
        if (optional.isEmpty()) {
            Boolean[][] pixelData = new Boolean[0][0];
            CACHE.put(texture, pixelData);
            LOG.error("Failed to load texture: " + texture);
            return pixelData;
        }

        try (InputStream input = optional.get().open()) {
